...
import { FistCode, EventGender, UserLogin } from "../ObjectCommon/EventCommon";
...
function Gender() {
  ...
  useEffect(() => {
    ...
    const CheckTokenAndRole = async () => {
      ...
      // Get Event Code
      var eventCode = ConcatStringEvent(FistCode, EventGender);
      ...
      if (resultCheckRole.Status === false) {
        ...
        dispatch(OldURLReducer.actions.AddUrl("/gender"));
        window.localStorage.setItem("oldURL", "/gender");

        // Setting Title Page
        document.title = "Gener";
